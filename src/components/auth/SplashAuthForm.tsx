...
export const SplashAuthForm = ({
  ...
  return <div ref={containerRef} className="absolute inset-0 z-20 bg-transparent flex flex-col items-center justify-start p-6 overflow-y-auto min-h-screen" style={{
    ...
    minHeight: '100vh'
      ...
      {/* Help Button */}
      <button onClick={() => setShowHelp(true)} className="absolute right-6 w-10 h-10 rounded-full bg-white/10 hover:bg-white/20 backdrop-blur-sm border border-white/30 flex items-center justify-center text-white transition-all" style={{
      ...
      <Dialog open={showHelp} onOpenChange={setShowHelp}>
        <DialogContent className="bg-white/10 backdrop-blur-lg border-white/30 w-[90vw] max-w-md animate-scale-in rounded-2xl">
          <DialogHeader className="space-y-3">
            <DialogTitle className="text-2xl text-white/90 font-semibold">Choose Your Experience</DialogTitle>
          ...
          <div className="space-y-8 pt-6">
            <div className="space-y-3 animate-fade-in">
              <div className="flex items-center gap-3">
                ...
                <h3 className="font-semibold text-xl text-white">Create and Discover</h3>
              </div>
            ...
            <div className="space-y-3 animate-fade-in">
              <div className="flex items-center gap-3">
                ...
                <h3 className="font-semibold text-xl text-white">Discover Only</h3>
              </div>
              <p className="text-sm text-white/70 leading-relaxed pl-13">
      ...
      {/* Main Auth Options */}
      {step === 'main' && <div className="w-full max-w-md space-y-6 animate-scale-in">
          {/* Primary Actions */}
          ...
          <div className="text-center">
            <Button variant="link" onClick={() => setStep('signin')} className="text-white/90 hover:text-white underline">
      ...
      {/* Creator Setup */}
      {step === 'creator-setup' && <div className="w-full max-w-md space-y-6 animate-scale-in">
          ...
          <div className="space-y-4">
            <Button onClick={() => handleSocialAuth('google')} className="w-full h-14 text-base font-semibold bg-white hover:bg-white/90 text-gray-900 transition-all" disabled={isLoading || loading}>
        ...
        }} className="w-full h-14 text-base font-semibold bg-white/5 hover:bg-white/10 text-white border-white/30 transition-all" disabled={isLoading || loading}>
              <Mail className="mr-3 h-5 w-5" />
              Sign up with Email
            </Button>
          </div>

          <Button variant="link" onClick={() => setStep('main')} className="w-full text-white/90 hover:text-white">
            ← Back
          </Button>
        </div>}

      {/* User Setup */}
      {step === 'user-setup' && <div className="w-full max-w-md space-y-6 animate-scale-in">
          <div className="text-center mb-8">
      ...
      {/* Sign In */}
      {step === 'signin' && <div className="w-full max-w-md space-y-6 animate-scale-in">
          <div className="text-center mb-8">
      ...
      {step === 'creator-email' && <div className="w-full max-w-md space-y-6 animate-scale-in">
          <div className="text-center mb-8">
            <TowerControl className="h-16 w-16 text-white mx-auto mb-4" />
            <h2 className="text-3xl font-bold text-white mb-2">Join as Creator</h2>
          ...
          <form onSubmit={handleEmailSignUp} className="space-y-4">
            <div className="space-y-2">
              ...
              <Input id="fullName" type="text" value={fullName} onChange={e => setFullName(e.target.value)} onFocus={e => ensureVisible(e.currentTarget)} placeholder="Enter your full name" className="bg-white/10 border-white/30 text-white placeholder:text-white/50" required />
            </div>
            
            <div className="space-y-2">
              <Label htmlFor="email" className="text-white">Email</Label>
              <Input id="email" type="email" value={email} onChange={e => setEmail(e.target.value)} onFocus={e => ensureVisible(e.currentTarget)} placeholder="Enter your email" className="bg-white/10 border-white/30 text-white placeholder:text-white/50" required />
            </div>

            <div className="space-y-2">
              <Label htmlFor="password" className="text-white">Password</Label>
              <Input id="password" type="password" value={password} onChange={e => setPassword(e.target.value)} onFocus={e => ensureVisible(e.currentTarget)} placeholder="Create a password (min 6 characters)" className="bg-white/10 border-white/30 text-white placeholder:text-white/50" required minLength={6} />
            </div>

            {formError && <p className="text-sm text-red-300 bg-red-500/20 p-3 rounded-md">{formError}</p>}

            <Button type="submit" className="w-full h-14 text-base font-semibold bg-white hover:bg-white/90 text-gray-900 transition-all" disabled={isLoading || loading}>
              {isLoading ? <Loader2 className="mr-2 h-5 w-5 animate-spin" /> : 'Create Account'}
            </Button>
          </form>

          <Button variant="link" onClick={() => setStep('creator-setup')} className="w-full text-white/90 hover:text-white">
            ← Back
          </Button>
        </div>}

      {/* User Email Signup */}
      {step === 'user-email' && <div className="w-full max-w-md space-y-6 animate-scale-in">
          <div className="text-center mb-8">
            <Binoculars className="h-16 w-16 text-white mx-auto mb-4" />
            <h2 className="text-3xl font-bold text-white mb-2">Join as User</h2>
            <p className="text-white/80">Create your account with email</p>
          ...
          <Button variant="link" onClick={() => setStep('user-setup')} className="w-full text-white/90 hover:text-white">
            ← Back
          </Button>
        </div>}

      {/* Email Sign In */}
      {step === 'signin-email' && <div className="w-full max-w-md space-y-6 animate-scale-in">
          <div className="text-center mb-8">
            <LogIn className="h-16 w-16 text-white mx-auto mb-4" />
            <h2 className="text-3xl font-bold text-white mb-2">Welcome Back</h2>
          ...
          <form onSubmit={handleEmailSignIn} className="space-y-4">
            ...
            <div className="space-y-2">
              <Label htmlFor="password" className="text-white">Password</Label>
              <Input id="password" type="password" value={password} onChange={e => setPassword(e.target.value)} onFocus={e => ensureVisible(e.currentTarget)} placeholder="Enter your password" className="bg-white/10 border-white/30 text-white placeholder:text-white/50" required />
            </div>
          ...
          <Button variant="link" onClick={() => setStep('signin')} className="w-full text-white/90 hover:text-white">
            ← Back
          </Button>
        </div>}
